...
interface ITeamManager {
  ...
  function teams(uint256 teamId)
    external
    view
    returns (uint256 n0, uint256 lMin, uint256 lMax, string memory flagURI, uint256 currentR);

  function members(uint256 teamId, address account)
    external
    view
    returns (uint8 status, uint64 joinedAt, uint64 eliminatedAt, uint64 cooldownEndsAt);
}
...
contract TeamEconomy is AccessControl, ReentrancyGuard, ITeamEconomyHooks {
  ...
  // Stage scalar S (1e18) with optional delayed activation
  uint256 public stageScalar; // current S (1e18)
  ...
  event Claimed(uint256 indexed teamId, address indexed account, uint256 amount);
  ...
  function setTeamManager(address teamManager_) external onlyRole(DEFAULT_ADMIN_ROLE) {
    teamManager = ITeamManager(teamManager_);
  }

  function getStageScalar() public view returns (uint256) {
    if (pendingStageScalar != 0 && block.timestamp >= stageScalarEffectiveAt) {
  ...
  function creditTeamWEDO(uint256 teamId, uint256 amount) external onlyRole(DISTRIBUTOR_ROLE) {
  ...
  function creditPersonalIDO(address account, uint256 amount) external onlyRole(DISTRIBUTOR_ROLE) {
    require(amount > 0, "zero amount");
  ...
  function getTeamL(uint256 teamId) public view returns (uint256 L) {
    (uint256 n0, uint256 lMin, uint256 lMax, , uint256 R) = teamManager.teams(teamId);
    if (n0 == 0) return 0;
    uint256 S = getStageScalar();
    // L = S * (lMin + (lMax - lMin) * R / n0)
  ...
  function withdraw(uint256 teamId, uint256 amountWEDO) public nonReentrant {
    ...
    require(R > 0, "R=0");
    uint256 L = getTeamL(teamId); // already includes S and fixed point 1e18

    // burn WEDO and update balance
    teamWedoBalance[teamId] -= amountWEDO;
    wedo.burn(address(this), amountWEDO);

    // mintable IDO = amountWEDO * L / 1e18
    uint256 mintIdo = (amountWEDO * L) / 1e18;
  ...
  function pendingIdo(uint256 teamId, address account) public view returns (uint256) {
  ...
  function claim(uint256 teamId) public nonReentrant {
    ...
    if (amount == 0) {
      ...
      userRewardDebt[teamId][msg.sender] = userShares[teamId][msg.sender] * accIdoPerSurvivor[teamId];
      return;
    }
